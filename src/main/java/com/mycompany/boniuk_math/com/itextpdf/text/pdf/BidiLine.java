...
public class BidiLine {
  ...
  public boolean getParagraph(int runDirection) {
    ...
    if (runDirection == 2 || runDirection == 3) {
      ...
      BidiOrder order = new BidiOrder(this.text, 0, this.totalTextLength, (byte)((runDirection == 3) ? 1 : 0));
        ...
        if (width - charWidth < 0.0F)
          if (lastValidChunk == null && ck.isImage()) {
            ...
            if (img.isScaleToFitLineWhenOverflow()) {
              float scalePercent = width / img.getWidth() * 100.0F;
      ...
      if (he != null) {
        ...
        if (word != null) {
          ...
          if (pre.length() > 0) {
            ...
            return new PdfLine(0.0F, originalWidth, testWidth - lastValidChunk.font().width(pre), alignment, false, createArrayOfPdfChunks(oldCurrentChar, word[0] - 1, extra), isRTL);
          } 
        } 
      } 
    } 
  ...
  public int[] getWord(int startIdx, int idx) {
    int last = idx;
    int first = idx;
    for (; last < this.totalTextLength && 
      Character.isLetter(this.text[last]); last++);
    if (last == idx)
      return null; 
    for (; first >= startIdx && 
      Character.isLetter(this.text[first]); first--);
    first++;
...
/* Location:              C:\Users\windo\Desktop\itextpdf-5.1.1.jar!\com\itextpdf\text\pdf\BidiLine.class
