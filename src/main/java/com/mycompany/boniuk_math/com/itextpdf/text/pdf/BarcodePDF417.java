...
public class BarcodePDF417 {
  ...
  protected void outCodeword17(int codeword) {
    ...
    int bit = this.bitPtr - bytePtr * 8;
    this.outBits[bytePtr++] = (byte)(this.outBits[bytePtr++] | codeword >> 9 + bit);
    this.outBits[bytePtr++] = (byte)(this.outBits[bytePtr++] | codeword >> 1 + bit);
    codeword <<= 8;
    this.outBits[bytePtr] = (byte)(this.outBits[bytePtr] | codeword >> 1 + bit);
    this.bitPtr += 17;
  }
  ...
  protected void outCodeword18(int codeword) {
    int bytePtr = this.bitPtr / 8;
    int bit = this.bitPtr - bytePtr * 8;
    this.outBits[bytePtr++] = (byte)(this.outBits[bytePtr++] | codeword >> 10 + bit);
    this.outBits[bytePtr++] = (byte)(this.outBits[bytePtr++] | codeword >> 2 + bit);
    codeword <<= 8;
    this.outBits[bytePtr] = (byte)(this.outBits[bytePtr] | codeword >> 2 + bit);
    if (bit == 7)
      this.outBits[++bytePtr] = (byte)(this.outBits[++bytePtr] | 0x80); 
    this.bitPtr += 18;
  }
  ...
  protected void outPaintCode() {
    ...
    if ((this.options & 0x80) != 0)
      for (int k = 0; k < this.outBits.length; k++)
        this.outBits[k] = (byte)(this.outBits[k] ^ 0xFF);  
  }
  ...
  protected void calculateErrorCorrection(int dest) {
    ...
    int lastE = Alength - 1;
    int i;
    for (i = 0; i < this.lenCodewords; i++) {
      int t1 = this.codewords[i] + this.codewords[dest];
  ...
  private void textCompaction(byte[] input, int start, int length) {
    ...
    for (int k = start; k < length; k++) {
      ...
      if ((v & mode) != 0) {
        ...
        if ((ptr & 0x1) != 0) {
          dest[ptr++] = 29;
          mode = ((mode & 0x80000) != 0) ? 65536 : mode;
        } 
        dest[ptr++] = 913;
        dest[ptr++] = v & 0xFF;
        fullBytes += 2;
        ...
        switch (mode) {
          case 65536:
            ...
            if ((getTextTypeAndValue(input, length, k + 1) & getTextTypeAndValue(input, length, k + 2) & 0x80000) != 0) {
              dest[ptr++] = 28;
              dest[ptr++] = 25;
              dest[ptr++] = v & 0xFF;
              mode = 524288;
          ...
          case 131072:
            if ((v & 0x10000) != 0) {
              if ((getTextTypeAndValue(input, length, k + 1) & getTextTypeAndValue(input, length, k + 2) & 0x10000) != 0) {
                dest[ptr++] = 28;
                dest[ptr++] = 28;
                mode = 65536;
              } else {
  ...
  protected void textCompaction(int start, int length) {
    textCompaction(this.text, start, length);
  }
  
  protected void basicNumberCompaction(int start, int length) {
    basicNumberCompaction(this.text, start, length);
  }
  ...
  private void basicNumberCompaction(byte[] input, int start, int length) {
    ...
    for (int ni = start; ni < length; ni++) {
      for (k = retLast; k >= 0; k--)
        this.codewords[ret + k] = this.codewords[ret + k] * 10; 
      this.codewords[ret + retLast] = this.codewords[ret + retLast] + input[ni] - 48;
      for (k = retLast; k > 0; k--) {
        this.codewords[ret + k - 1] = this.codewords[ret + k - 1] + this.codewords[ret + k] / 900;
        this.codewords[ret + k] = this.codewords[ret + k] % 900;
      } 
    } 
  }
  ...
  protected void numberCompaction(int start, int length) {
    numberCompaction(this.text, start, length);
  }
  ...
  protected void byteCompaction6(int start) {
    ...
    for (int ni = start; ni < length; ni++) {
      for (k = retLast; k >= 0; k--)
        this.codewords[ret + k] = this.codewords[ret + k] * 256; 
      this.codewords[ret + retLast] = this.codewords[ret + retLast] + (this.text[ni] & 0xFF);
      for (k = retLast; k > 0; k--) {
        this.codewords[ret + k - 1] = this.codewords[ret + k - 1] + this.codewords[ret + k] / 900;
        this.codewords[ret + k] = this.codewords[ret + k] % 900;
      } 
  ...
  void breakString() {
        ...
        if (nd >= 13) {
          if (lastP != startN) {
            ...
            for (int j = lastP; j < startN; j++) {
              ...
              boolean txt = ((c >= ' ' && c < '') || c == '\r' || c == '\n' || c == '\t');
              if (txt != lastTxt) {
                this.segmentList.add(lastTxt ? 84 : 66, lastP, j);
                lastP = j;
                lastTxt = txt;
              } 
            } 
            this.segmentList.add(lastTxt ? 84 : 66, lastP, startN);
          } 
    ...
    for (k = 0; k < this.segmentList.size(); k++) {
      Segment segment1 = this.segmentList.get(k);
      Segment vp = this.segmentList.get(k - 1);
      Segment vn = this.segmentList.get(k + 1);
      if (checkSegmentType(segment1, 'B') && getSegmentLength(segment1) == 1 && 
        checkSegmentType(vp, 'T') && checkSegmentType(vn, 'T') && getSegmentLength(vp) + getSegmentLength(vn) >= 3) {
        vp.end = vn.end;
    ...
    for (k = 0; k < this.segmentList.size(); k++) {
      ...
      Segment vp = this.segmentList.get(k - 1);
      Segment vn = this.segmentList.get(k + 1);
      if (checkSegmentType(segment1, 'T') && getSegmentLength(segment1) >= 5) {
        boolean redo = false;
        if ((checkSegmentType(vp, 'B') && getSegmentLength(vp) == 1) || checkSegmentType(vp, 'T')) {
          redo = true;
          segment1.start = vp.start;
    ...
    Segment v;
    if (this.segmentList.size() == 1 && (v = this.segmentList.get(0)).type == 'T' && getSegmentLength(v) >= 8) {
    ...
    for (int k = 0; k < this.segmentList.size(); k++) {
      ...
      switch (v.type) {
        ...
        case 'N':
          ...
          break;
        case 'B':
          this.codewords[this.cwPtr++] = (getSegmentLength(v) % 6 != 0) ? 901 : 924;
  ...
  public void paintCode() {
    ...
    boolean fixedColumn = ((this.options & 0x4) == 0);
    ...
    } else if ((this.options & 0x6) == 0) {
      fixedColumn = true;
      if (this.aspectRatio < 0.001D) {
        this.aspectRatio = 0.001F;
      } else if (this.aspectRatio > 1000.0F) {
        this.aspectRatio = 1000.0F;
      } 
      double b = (73.0F * this.aspectRatio - 4.0F);
      double c = (-b + Math.sqrt(b * b + (68.0F * this.aspectRatio * (this.lenCodewords + lenErr) * this.yHeight))) / (34.0F * this.aspectRatio);
      this.codeColumns = (int)(c + 0.5D);
      if (this.codeColumns < 1) {
        this.codeColumns = 1;
      } else if (this.codeColumns > 30) {
        this.codeColumns = 30;
  ...
  public Image getImage() throws BadElementException {
    paintCode();
    byte[] g4 = CCITTG4Encoder.compress(this.outBits, this.bitColumns, this.codeRows);
    return Image.getInstance(this.bitColumns, this.codeRows, false, 256, ((this.options & 0x80) == 0) ? 0 : 1, g4, null);
  }
  ...
  public Image createAwtImage(Color foreground, Color background) {
    ...
    int[] pix = new int[this.bitColumns * this.codeRows * h];
    int stride = (this.bitColumns + 7) / 8;
    int ptr = 0;
    for (int k = 0; k < this.codeRows; k++) {
      int p = k * stride;
      int j;
      for (j = 0; j < this.bitColumns; j++) {
    ...
    Image img = canvas.createImage(new MemoryImageSource(this.bitColumns, this.codeRows * h, pix, 0, this.bitColumns));
    return img;
  }
  ...
  private static final int[][] CLUSTERS = new int[][] { { 
  ...
  private static final int[][] ERROR_LEVEL = new int[][] { { 27, 917 }, { 522, 568, 723, 809 }, { 237, 308, 436, 284, 646, 653, 428, 379 }, { 
  ...
  protected static class SegmentList {
    protected ArrayList<BarcodePDF417.Segment> list = new ArrayList<BarcodePDF417.Segment>();
...
/* Location:              C:\Users\windo\Desktop\itextpdf-5.1.1.jar!\com\itextpdf\text\pdf\BarcodePDF417.class
