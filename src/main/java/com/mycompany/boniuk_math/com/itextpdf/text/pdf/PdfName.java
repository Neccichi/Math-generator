...
public class PdfName extends PdfObject implements Comparable<PdfName> {
  ...
  static {
    ...
    try {
      for (int fldIdx = 0; fldIdx < fields.length; fldIdx++) {
        ...
        if ((curFld.getModifiers() & 0x19) == 25 && curFld.getType().equals(PdfName.class)) {
  ...
  public PdfName(String name, boolean lengthCheck) {
    ...
    if (lengthCheck && length > 127)
      throw new IllegalArgumentException(MessageLocalization.getComposedMessage("the.name.1.is.too.long.2.characters", new Object[] { name, String.valueOf(length) })); 
...
/* Location:              C:\Users\windo\Desktop\itextpdf-5.1.1.jar!\com\itextpdf\text\pdf\PdfName.class
